  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-neutral-900 dark:text-white">Pendidikan</h1>
          <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">{items.length} item</p>
        </div>
        <Link
          href="/dashbor/educations/new"
      ...
      {items.length > 0 ? (
          ...
          {items.map((item) => (
            <div key={item.id} className="flex items-center justify-between gap-4 px-4 py-3">
              <div className="min-w-0">
                ...
                <p className="text-sm text-neutral-500 dark:text-neutral-400 truncate">{item.institution}</p>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <Link
                  href={`/dashbor/educations/${item.id}`}
        ...
        <div className="rounded-xl border border-dashed border-neutral-300 dark:border-neutral-700 p-8 text-center">
          <p className="text-sm text-neutral-500 dark:text-neutral-400 mb-3">Belum ada data pendidikan.</p>
          <Link
            href="/dashbor/educations/new"
